...
まず、各盤面にはたかだか25個のノードが内包されていると上で述べたが、それらの多くは到達不可能なノードである。ある盤面p上の合法手mについて、mが禁じ手となるようなノードに到達可能であることは、pからmを指して遷移した先の盤面p'から1手でpに戻るような指し手m'が存在することと同値だが、そのようなケースは割合としては少ないと考えられる。そこで、以下のような手順により非自明なノードを列挙することを考える。この意図は、その後に非自明なノードのみについて決着までの手数を求めたいからである。
