...
逆に言うと、上記の6マスのうち別の穴に配置した2つの盤面同士は、決して対称にならない。また手番側or相手のコマの数が異なる盤面同士も決して対称にならない。ゆえに、『穴の位置6通り * コマの数4通り = 24通り』という場合分けにより、全局面を排他的かつ網羅的に分割できるといえる。対称な局面の検出もその分割の内側同士でだけ考慮すればよい。
