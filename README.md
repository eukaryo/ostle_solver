...
もう一つのobservationとして、上記の6マスのうち異なるマスに穴を配置した2つの盤面同士は決して対称にならない。そしてコマの数が異なる盤面同士も決して対称にならない。ゆえに、『穴の位置とコマの数』の場合分け（24通り）によって、全盤面を網羅しつつ相互排他的に分割できる。加えて、対称な盤面の検出もその分割の内側同士でだけ考慮すればよくなる。
